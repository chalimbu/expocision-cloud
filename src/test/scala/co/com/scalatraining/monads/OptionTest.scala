...
class OptionSuite extends AnyFunSuite {
  ...
  test("Se debe poder hacer pattern match sobre un Option") {
    ...
    def brandType(brand : Option[String]) = brand match {
      case Some(  brand ) => brand
      case None => "not specified"
    }

    var value = brandType(cars(1).brand)
    assert(value == "Sandero")

    value = brandType(cars(3).brand)
  ...
  test("Componer varios Option en un for-comp. Caso todos Some") {
    val value1 = Option(1)
